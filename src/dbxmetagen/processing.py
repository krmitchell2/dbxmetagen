...
def chunk_df(config, df: DataFrame, columns_per_call: int = 5) -> List[DataFrame]:
    """
...
def claim_table(table_name: str, config: MetadataConfig, max_retries: int = 3) -> bool:
    # TODO: change to distributed lock
    """
...
def get_generated_metadata_data_aware(
    ...
    responses = []
    nrows = df.count()
    chunked_dfs = chunk_df(config, df, config.columns_per_call)
...
def create_tables(config: MetadataConfig) -> None:
    # makes a single table used to track which tables have been processed
    """
    Creates a schema volume if it does not already exist.
...
def setup_queue(config: MetadataConfig) -> List[str]:
    ...
    combined_table_names = list(
        ...
        set().union(queued_table_names, config_table_names)
